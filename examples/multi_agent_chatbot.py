    ...
    async def chat_completion(self, messages: List[Dict], temperature: float = 0.7):
        ...
        if "router" in self.model_name:
            ...
            if "code" in last_message.lower() or "programming" in last_message.lower():
                ...
                response = "creative"
            elif "complex" in last_message.lower() or "multi-step" in last_message.lower():
            ...
            response = f"Coordinator response: I'll break down '{last_message}' into steps"
        else:
...
class MultiAgentChatbot:
    ...
    def _setup_agents(self):
        ...
        router_config = AgentConfig(
            ...
            custom_headers=CustomHeaders(
                additional_headers={"agent_role": "router"}
            )
        )
        ...
        with AgentContext(specialist_agent):
            update_current_span(
                metadata={
                    ...
                    "specialist_capabilities": list(specialist_agent.config.capabilities),
    ...
    async def _process_creative_query(self, query: str) -> str:
        ...
        response = await self.llm_clients["creative"].chat_completion(messages, temperature=0.9)
        return response.choices[0].message.content
    ...
    async def _process_code_query(self, query: str) -> str:
        ...
        response = await self.llm_clients["code"].chat_completion(messages, temperature=0.3)
        return response.choices[0].message.content
    
    @llm_trace(
        model="coordinator-gpt-4",
        operation="chat",
        ai_system="openai"
    )
    ...
    def get_conversation_stats(self) -> Dict:
        ...
        stats = {
            ...
            "average_response_length": total_response_length / len(self.conversation_history),
            "conversation_duration": time.time() - self.conversation_history[0]["timestamp"]
        }
        
        update_current_span(
            output=stats,
            metadata={"stats_generated": True}
        )
    ...
    noveum_trace.init(
        ...
        sinks=[
            ...
            FileSink(FileSinkConfig(
                file_path="chatbot_traces.jsonl",
                max_file_size_mb=10
            ))
        ]
    )
