...
from noveum_trace import (
    Agent, AgentConfig, get_agent_registry,
...
def chat_completion(messages: List[Dict]) -> MockLLMResponse:
    ...
    response_content = f"AI: I understand you said '{last_message}'. Here's my response."
...
async def create_embedding(text: str) -> List[float]:
    ...
    embedding = [0.1 * i for i in range(1536)]  # 1536-dimensional vector
...
def custom_llm_call(prompt: str, max_tokens: int = 100) -> MockLLMResponse:
    ...
    response_content = f"Claude: {prompt[:50]}... [Generated response with {max_tokens} max tokens]"
...
def extract_text(document: Dict) -> str:
    ...
    extracted_text = f"Extracted text from {document.get('type', 'unknown')} document: {content}"
...
async def run_multi_agent_workflow(task: str) -> Dict:
    ...
    update_current_span(
        output=results,
        metadata={
            "agents_used": ["processor", "analyzer"],
...
async def main():
    ...
    multi_agent_result = await run_multi_agent_workflow("Analyze customer feedback data")
