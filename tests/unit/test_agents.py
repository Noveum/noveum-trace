...
class TestAgentConfig:
    ...
    def test_agent_config_with_custom_headers(self):
        ...
        config = AgentConfig(
            name="test-agent", agent_type="llm", custom_headers=headers
        )
    ...
    def test_agent_config_validation_empty_name(self):
        ...
        with pytest.raises(
            ValidationError, match="Agent name must be a non-empty string"
        ):
            config.validate()
    ...
    def test_agent_config_validation_invalid_sampling_rate(self):
        ...
        with pytest.raises(
            ValidationError, match="Sampling rate must be between 0.0 and 1.0"
        ):
            config.validate()
...
class TestAgent:
    ...
    def test_agent_with_project_tracer_config(self):
        ...
        project_config = TracerConfig(
            project_id="test-project", project_name="test-project", sampling_rate=0.5
        )
...
class TestAgentRegistry:
    ...
    def test_agent_registration_duplicate_name(self):
        ...
        with pytest.raises(
            ValidationError, match="Agent with name 'test-agent' already exists"
        ):
    ...
    def test_parent_child_relationships(self):
        ...
        child_config = AgentConfig(
            name="child-agent", agent_type="worker", parent_agent="parent-agent"
        )
