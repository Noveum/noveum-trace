...
class TestWrappedSpeechStream:
    ...
    @patch("noveum_trace.integrations.livekit.livekit.create_span_attributes")
    @pytest.mark.asyncio
    async def test_anext_final_transcript_creates_span(
        self,
        mock_create_attrs,
        mock_calc_duration,
        mock_gen_filename,
        ...
        """Test __anext__ creates span on FINAL_TRANSCRIPT event."""
        from noveum_trace.integrations.livekit.livekit import _WrappedSpeechStream

        # Mock SpeechEventType - patch at the import location
        ...
        with patch(
            ...
            mock_alternative = Mock()
            mock_alternative.text = "Test transcript"
            ...
            # Add some buffered frames
            stream._buffered_frames = [Mock(), Mock()]

            mock_get_trace.return_value = mock_trace
            mock_get_client.return_value = mock_client
            mock_gen_filename.return_value = "stt_0001_123.wav"
            mock_calc_duration.return_value = 2000.0
            mock_create_attrs.return_value = {"stt.provider": "deepgram"}
