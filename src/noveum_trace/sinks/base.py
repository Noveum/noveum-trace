"""
Base sink interface for the Noveum Trace SDK.
"""
...
from ..utils.exceptions import SinkError, ConfigurationError
...
class BaseSink(ABC):
    ...
    def __init__(self, config: SinkConfig):
        ...
        try:
            ...
            raise SinkError(f"Sink initialization failed: {e}", sink_name=self.name) from e
    ...
    def send_batch(self, spans: List[SpanData]) -> None:
        ...
        try:
            ...
            logger.debug(f"Successfully sent {len(spans)} spans to sink '{self.name}' in {latency_ms:.2f}ms")
            ...
            if self._status == SinkStatus.HEALTHY:
                ...
                self._status = SinkStatus.UNHEALTHY
                logger.error(f"Sink '{self.name}' marked unhealthy due to repeated errors: {e}")
            
            # Re-raise the exception
            raise SinkError(f"Failed to send batch to sink '{self.name}': {e}", sink_name=self.name) from e
        ...
        try:
            ...
            if result:
                if self._status == SinkStatus.UNHEALTHY:
                    self._status = SinkStatus.DEGRADED
                    logger.info(f"Sink '{self.name}' health check passed, status upgraded to degraded")
                elif self._status == SinkStatus.DEGRADED:
                    self._status = SinkStatus.HEALTHY
                    logger.info(f"Sink '{self.name}' health check passed, status upgraded to healthy")
            return result
        except Exception as e:
        ...
        for attempt in range(self._config.max_retries + 1):
            try:
                ...
                if attempt < self._config.max_retries:
                    delay_ms = self._config.retry_delay_ms * (2 ** attempt)  # Exponential backoff
                    logger.warning(
                    ...
                    logger.error(f"All {self._config.max_retries + 1} attempts failed for sink '{self.name}'")
