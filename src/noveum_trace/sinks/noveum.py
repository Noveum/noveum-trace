"""
Noveum.ai sink for the Noveum Trace SDK.

This sink will send trace data to the Noveum.ai platform for real-time evaluation
and dataset creation. Currently a placeholder implementation.
"""
...
from ..utils.exceptions import NoveumAPIError, ConfigurationError
...
class NoveumConfig(SinkConfig):
    ...
    def traces_endpoint(self) -> str:
        ...
        return f"{self.api_base_url}/{self.api_version}/projects/{self.project_id}/traces"
...
class NoveumSink(BaseSink):
    ...
    def __init__(self, config: NoveumConfig):
        ...
        if not AIOHTTP_AVAILABLE:
            logger.warning("aiohttp not available. Noveum sink will use placeholder implementation.")
    ...
    def _initialize(self) -> None:
        ...
        logger.info(f"Noveum sink initialized for project: {self._noveum_config.project_id}")
    ...
    def _test_connectivity(self) -> None:
        """Test API connectivity (placeholder)."""
        logger.debug(f"Testing connectivity to Noveum.ai for project {self._noveum_config.project_id}")
    ...
    def _send_batch(self, spans: List[SpanData]) -> None:
        ...
        logger.info(f"Sending {len(spans)} spans to Noveum.ai (placeholder implementation)")
    ...
    def _test_connectivity(self) -> None:
        """Test connectivity to Noveum API."""
        # TODO: Implement actual connectivity test
        # For now, just log a placeholder message
        logger.info(f"Testing connectivity to {self._noveum_config.api_base_url} (placeholder)")
    ...
    def _trigger_evaluation(self, spans: List[SpanData]) -> None:
        ...
        for span in llm_spans:
            ...
            logger.debug(f"Evaluation request for span {span.span_id}: {evaluation_request}")
    ...
    def _update_dataset(self, spans: List[SpanData]) -> None:
        ...
        dataset_name = self._noveum_config.dataset_name or f"traces-{self._noveum_config.project_id}"
    ...
    def _extract_llm_data(self, span: SpanData) -> Optional[Dict[str, Any]]:
        ...
        for event in span.events:
            if event.get("name") in ["gen_ai.content.prompt", "gen_ai.content.completion"]:
    ...
    def _is_llm_span(self, span: SpanData) -> bool:
        ...
        for event in span.events:
            if event.get("name", "").startswith("gen_ai."):
                return True
