...
class ConsoleSink(BaseSink):
    ...
    def __init__(self, config: ConsoleSinkConfig = None):
    ...
    def _print_span(self, span: SpanData) -> None:
        ...
        # Apply filters
        if span.duration_ms < self._config.min_duration_ms:
            return
        
        if self._config.include_successful_only and span.status != "ok":
            return
    ...
    def _print_pretty_span(self, span: SpanData) -> None:
        ...
        # Error details
        if span.status == "error" and "status.description" in span.attributes:
