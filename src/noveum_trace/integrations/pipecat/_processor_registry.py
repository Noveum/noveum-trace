...
Processor display names are not unique: applications can construct two instances of
the same provider with the same configured name.  This registry therefore keeps the
actual processor object as the identity and uses exact names only as a fallback for
metric records that do not carry an object reference.
"""
    ...
    def resolve_metric_processors(
        ...
        if source is not None:
            ...
            if required_role is not None and not source_record.has_role(required_role):
                # An exact, known source must never fall through to an unrelated
                # processor merely because that processor has the requested role.
        ...
        # Return every role candidate. The typed router, which also knows active
        # operations, decides whether this is an exact, zero, or multiple match.
        # Generic TTFB/processing metrics pass required_role=None and never use
        # this fallback.
