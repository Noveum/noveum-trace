...
class NormalizedMetricData:
    """One Pipecat metric item, preserved independently of its frame siblings.

    ``value`` deliberately remains family-specific: scalar metric families retain
    their scalar value, while usage and turn metrics retain a structured mapping.
    Routing and unit conversion happen later, after the emitting processor and the
    owning operation have been resolved.
    """
...
def _metric_item_metadata(item: Any) -> tuple[str | None, str | None]:
    """Return the processor and model reported by a native metric item."""
...
def _structured_metric_value(item: Any, fields: tuple[str, ...]) -> dict[str, Any]:
    """Preserve the present fields of a non-scalar metric item, including zeroes."""
    return {
...
def _unknown_metric_value(item: Any) -> Any:
    """Best-effort diagnostic value for an unsupported Pipecat metric class."""
    ...
    try:
        ...
        logger.debug("Failed to preserve unknown metric value: %s", e)
    return None
...
def normalize_metrics_data(frame: Any) -> list[NormalizedMetricData]:
    ...
    The function intentionally performs no operation attribution and no rollup.
    In particular, generic TTFB and processing records are not classified as LLM,
    STT, or TTS metrics here. Unknown metric classes remain visible as ``unknown``
    records so callers can emit bounded diagnostic telemetry.
    """
    ...
    try:
        ...
        optional_types: dict[str, type[Any] | None] = {}
        for class_name in (
            ...
            try:
                ...
                optional_types[class_name] = getattr(pipecat_metrics, class_name, None)
            except ImportError:
                optional_types[class_name] = None
        ...
        logger.debug("Failed to normalize metrics data: %s", e)
